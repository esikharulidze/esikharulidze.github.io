...
const GroupTherapy: FC<PageSubcriptionProps> = ({ className = '' }) => {
	...
	useEffect(() => {
		;(async () => {
			try {
				setIsLoading(true)
				await axios
					.get<any, AxiosResponse<BackendService>>('service/grouptherapy')
					.then(({ data }) => {
						// setServices(data)
	...
	return (
		<div className={`nc-PageSubcription ${className}`} data-nc-id='PageSubcription'>
			<LayoutPage
				...
				// headingEmoji="💎"
				heading={selectedService ? selectedService.title : 'ჯგუფური თერაპია'}
				isInner={false}
			>
