...
const NavMobile: React.FC<NavMobileProps> = ({
      ...
      <ul className="nav-mobile-sub-menu pl-6 pb-1 text-base">
        {item.children?.map((i, index) => (
          <Disclosure key={i.href + index} as="li">
            ...
            >
              <span onClick={onClickClose}>{i.name}</span>
              {i.children && (
                <span
    ...
    return (
      ...
      >
        <NavLink
          ...
          activeClassName="text-secondary"
        >
          <span onClick={onClickClose}>{item.name}</span>
          {item.children && (
            <span
              className="block flex-grow"
              onClick={(e) => e.preventDefault()}
  ...
  return (
    <div className="overflow-y-auto w-full max-w-md h-screen py-2 transition transform shadow-lg ring-1 dark:ring-neutral-700 bg-white dark:bg-neutral-900 divide-y-2 divide-neutral-100 dark:divide-neutral-800">
      <div className="py-6 px-5">
        <Logo />
        <div className="flex flex-col mt-5 text-neutral-700 dark:text-neutral-300 text-sm">
          <span>
          ანიმუსი - არის ახალი იდეა ფსიქოთერაპიულ სივრცეში. ჩვენ ვეხმარებით ადამიანს საკუთარი რესურსების აღმოჩენის გზით, ფსიქოლოგიური გამოწვევების გადაჭრაში და თვითგანვითარებაში
          </span>
